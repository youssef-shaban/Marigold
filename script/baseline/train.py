...
def instantiate_unet(cfg_node) -> UNet2DConditionModel:
    kwargs = OmegaConf.to_container(cfg_node.kwargs, resolve=True)
    return UNet2DConditionModel(**kwargs)


def instantiate_scheduler(cfg_node) -> DDPMScheduler:
    kwargs = OmegaConf.to_container(cfg_node.kwargs, resolve=True)
...
if "__main__" == __name__:
    ...
    parser = argparse.ArgumentParser(
        description="Baseline Conditional2DUnet Surface Normals Training"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="config/train_baseline_normals.yaml",
    ...
    logging.debug("Augmentation: %s", cfg.augmentation)
    ...
    pipeline_kwargs = (
        OmegaConf.to_container(cfg.pipeline.kwargs, resolve=True)
        if cfg.pipeline.kwargs is not None
        else {}
    )

    unet = instantiate_unet(cfg.model.unet)
    scheduler = instantiate_scheduler(cfg.model.scheduler)
